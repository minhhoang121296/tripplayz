import { useTranslations } from 'next-intl'

export const ChatTime = () => {
    const t = useTranslations('')

    return (
        <div className='w-full bg-black-darkBlack'>
            <div className='container mx-auto flex w-full flex-col gap-[50px] px-5 py-[44px] md:gap-[62px] md:py-[142.5px] xl:px-[218.5px]'>
                <div className='flex w-full flex-col gap-[30px] text-center text-white md:gap-[80px]'>
                    <p className='text-3xl font-semibold md:text-4xl'>
                        {t('responding_to_inquiries')}
                    </p>
                </div>
                <div className='flex flex-col items-center gap-[55px] md:flex md:flex-row md:gap-[55px]'>
                    <div className='flex flex-1 flex-col gap-4 text-center'>
                        <div className='pb-6 text-3xl text-white md:text-4xl'>
                            {t('chat')}
                        </div>
                        <div className='flex flex-col '>
                            <div className='flex flex-row border-y border-gray/50 py-[22px] text-white'>
                                <span className='flex-1 text-lg font-semibold'>
                                    {t('may')}
                                </span>
                                <span className='flex-1 text-lg font-semibold'>
                                    {t('june')}
                                </span>
                                <span className='flex-1 text-lg font-semibold'>
                                    {t('muly')}
                                </span>
                            </div>
                            <div className='flex flex-row border-b border-gray/50 pb-6 pt-1'>
                                <div className='flex flex-1 flex-col'>
                                    <span className='text-7xl text-blue drop-shadow-[0_4px_40px_#0055FF80]'>
                                        2
                                    </span>
                                    <span className='text-lg text-white'>
                                        {t('seconds')}
                                    </span>
                                </div>
                                <div className='flex flex-1 flex-col'>
                                    <span className='text-7xl text-blue drop-shadow-[0_4px_40px_#0055FF80]'>
                                        48
                                    </span>
                                    <span className='text-lg text-white'>
                                        {t('seconds')}
                                    </span>
                                </div>
                                <div className='flex flex-1 flex-col'>
                                    <span className='text-7xl text-blue drop-shadow-[0_4px_40px_#0055FF80]'>
                                        60
                                    </span>
                                    <span className='text-lg text-white'>
                                        {t('seconds')}
                                    </span>
                                </div>
                            </div>
                            <div className='text-basem pt-[34px] text-center text-white '>
                                {t('support_team_efficiency')}
                            </div>
                        </div>
                    </div>
                    <div className='flex flex-1 flex-col gap-4 text-center'>
                        <div className='pb-6 text-3xl text-white md:text-4xl'>
                            {t('Email')}
                        </div>
                        <div className='flex flex-col '>
                            <div className='flex flex-row border-y border-gray/50 py-[22px] text-white'>
                                <span className='flex-1 text-lg font-semibold'>
                                    {t('may')}
                                </span>
                                <span className='flex-1 text-lg font-semibold'>
                                    {t('june')}
                                </span>
                                <span className='flex-1 text-lg font-semibold'>
                                    {t('muly')}
                                </span>
                            </div>
                            <div className='flex flex-row border-b border-gray/50 pb-6 pt-1 '>
                                <div className='flex flex-1 flex-col'>
                                    <span className='text-7xl text-blueGray drop-shadow-[0_4px_40px_#A5BAE480]'>
                                        2
                                    </span>
                                    <span className='text-lg text-white'>
                                        {t('seconds')}
                                    </span>
                                </div>
                                <div className='flex flex-1 flex-col'>
                                    <span className='text-7xl text-blueGray drop-shadow-[0_4px_40px_#A5BAE480]'>
                                        48
                                    </span>
                                    <span className='text-lg text-white'>
                                        {t('seconds')}
                                    </span>
                                </div>
                                <div className='flex flex-1 flex-col'>
                                    <span className='text-7xl text-blueGray drop-shadow-[0_4px_40px_#A5BAE480]'>
                                        60
                                    </span>
                                    <span className='text-lg text-white'>
                                        {t('seconds')}
                                    </span>
                                </div>
                            </div>
                            <div className='text-basem pt-[34px] text-center text-white '>
                                {t('email_response_time')}
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    )
}
